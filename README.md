# 📊 My Power BI Reports Portfolio

## Report 1: Sales Performance Dashboard

### Description
This dashboard tracks sales performance across regions, products, and time periods. Key metrics include total sales, profit, and top-selling countries.

### Key Insights
- Total profit: $16,89M
- Best performing region: Canada

### Screenshot
<img width="1426" height="798" alt="image" src="https://github.com/user-attachments/assets/b8a4d4e6-6910-417c-ac7a-e512a322c43e" />

### Download
[Sales Dashboard.pbix](files/Sales_Report.pbix)

---

## Report 2: Sales Performance Dashboard

### Description
This dashboard tracks sales performance with multiple slicers.

### Key Insights
- Sum of sales: 118,73M
- Units sold 1,13M

### Screenshot
<img width="1425" height="799" alt="image" src="https://github.com/user-attachments/assets/500349ec-bf80-4434-8746-d81d06e1aecd" />

### Download
[Download Churn Analysis.pbix](files/Sales Report 2.pbix)

---

## Report 3: Data job report

### Description
HR analytics dashboard showing salary rates by countries and job role.

### Key Insights
- Median Salary
- Department with highest attrition: Sales
- Highest salary data job: Machine learning engineer

### Screenshot
<img width="1310" height="733" alt="image" src="https://github.com/user-attachments/assets/291c4244-48b7-4871-8bc7-68279995b83b" />

### Download
[Download HR Attrition Report.pbix](files/Job Report.pbix)

---
